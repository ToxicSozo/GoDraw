...
const (
	...
	codePullRequestExists = "PR_EXISTS"
	codePullRequestMerged = "PR_MERGED"
	codeReviewerMissing   = "NOT_ASSIGNED"
...
func (s *Server) handleCreatePullRequest(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, store.ErrPullRequestExists):
			writeError(w, http.StatusConflict, codePullRequestExists, "pull request id already exists")
...
func (s *Server) handleReassign(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, store.ErrPullRequestMerged):
			writeError(w, http.StatusConflict, codePullRequestMerged, "cannot reassign on merged PR")
		case errors.Is(err, store.ErrReviewerNotAssigned):
			writeError(w, http.StatusConflict, codeReviewerMissing, "reviewer is not assigned to this PR")
...
func badRequest(w http.ResponseWriter, message string) {
	writeError(w, http.StatusBadRequest, codeNotFound, message)
}
...
func internalError(w http.ResponseWriter) {
	writeError(w, http.StatusInternalServerError, codeNotFound, "internal server error")
}
